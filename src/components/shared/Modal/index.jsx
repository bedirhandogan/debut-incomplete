...
function Modal({ children, style, title }) {
  ...
  const handleClick = useCallback(
    (event) => {
      if (event.composedPath().includes(ref.current) && !event.composedPath().includes(refWrapper.current))
        dispatch(
          change({
            active: false,
          }),
  ...
  return (
    <div className={'modal'} style={{ display: data.active ? 'flex' : 'none' }} ref={ref}>
      <div className={'modal-main'} style={style} ref={refWrapper}>
        <div className={'modal-main-header'}>
          ...
          >
            <IconX stroke={1.5} width={20} height={20} style={{ color: 'var(--icon-color-primary)' }} />
          </div>
        </div>
        {children}
      </div>
